...
Explicitly out of scope (no grammar rule, flagged in code comments where relevant): `Not`/`And`/`Or`/`Xor` boolean/logical operators, `ScalarNil`/`Abort`/`Len`, `AppendTable`/`PopTable` (no agreed surface syntax), `Function`/`NativeFunction`-as-value (function-pointer literal syntax).
