...
Handles: `fn NAME(params) { ... }` with parameters, `return EXPR;`, `global NAME = EXPR;` (`global_var_statement`), call expressions, binary expressions (`<= < == != + - * /` with precedence tiers via `prec.left`), and `if COND { EXPR }` with an optional `else { EXPR }` as an expression (`if_expression` / `expression_block` — maps to `IfTrue` when `else` is omitted, `IfElse` when present). Confirmed against `test/fibonacci_program_recursive.caol` (`test/corpus/functions.txt`) and precedence/else-less-if cases (`test/corpus/expressions.txt`). `if_expression`'s branches are still single expressions (`expression_block`), not statement lists — a bare `if { ... }` used as a standalone statement (not wrapped in `return`) isn't supported yet since `_statement` has no generic expression-statement. Still unimplemented — marked with `// TODO` comments in the file: local `NAME = expr;` (`SetVar`, no `global`), `while`/`repeat`/`foreach`, arrays, string/float literals, dotted property access, closures, imports/submodules.
